...
const camel_case_properties = ['border-radius', 'border-color', 'border-top-color', 'border-bottom-color', 'background-color',
    'flex-direction', 'flex-shrink', 'flex-grow', 'flex-wrap',
    ...
    'text-align', 'overflow-x', 'overflow-y', 'num-columns', 'pointer-events', 'border-top-width', 'border-bottom-width',
    'letter-spacing'];
...
const propertiesSplitter = (property, number_value, value_string) => {
    ...
    if (property.indexOf('border') !== -1) {
        ...
        if (array.length === 1) {
            ...
            // } else {
                object[`${property}Top`] =  splitVariable(array[0]); // all properties are variables like 0.5rem and etc or 0
                object[`${property}Right`] = splitVariable(array[0]);
                object[`${property}Bottom`] = splitVariable(array[0]);
                object[`${property}Left`] = splitVariable(array[0]);
            // }
        }
    }
    // console.log(object);
